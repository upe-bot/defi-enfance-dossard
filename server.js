...
const REDIS_TTL_SEC = 30 * 24 * 60 * 60; // 30 jours
...
async function loadResultats() {
  ...
  // 1. Contacts : km + classement + dossard
  const contacts = [];
  let cursor = null;
  while (true) {
    await sleep(DELAY);
  ...
  // 2. Paiements billetterie : equipe + asso par contact_id
  const equipeParContactId = new Map();
  const assoParContactId   = new Map();
  let cursorP = null;
  while (true) {
    await sleep(DELAY);
    const url = cursorP
      ? `${OHME_BASE}/api/v1/payments?limit=250&since_date=2025-01-01&cursor=${encodeURIComponent(cursorP)}`
      : `${OHME_BASE}/api/v1/payments?limit=250&since_date=2025-01-01`;
    const res = await fetch(url, { headers: HEADERS });
    if (!res.ok) break;
    const json = await res.json();
    ...
    for (const p of items) {
      ...
      if (!eventName.includes('ANGERS')) continue;
      const equipe = (p.equipe || '').trim();
      if (equipe && equipe.toLowerCase() !== 'je cours solo') {
        equipeParContactId.set(String(p.contact_id), equipe);
      }
      const asso = (p.asso_soutenue || '').trim();
      if (asso) assoParContactId.set(String(p.contact_id), asso);
    }
    cursorP = json.cursor || null;
    if (!cursorP || items.length < 250) break;
  }

  // 3. Structures : km + classement équipes
  const structures = [];
  let cursorS = null;
  while (true) {
    await sleep(DELAY);
  ...
  // Construire classement coureurs avec équipe
  const classementCoureurs = contacts
    .map(c => ({
      ...
      equipe:     equipeParContactId.get(String(c.id)) || null,
      asso:       assoParContactId.get(String(c.id))   || null,
    }))
...
app.get('/api/resultats/refresh', async (req, res) => {
  try {
    ...
    await saveToRedis_key('defi_enfance_resultats', JSON.stringify(data), 7 * 24 * 60 * 60); // 7 jours
...
// Route seed Joué — injecte les dossards Joué-lès-Tours dans Redis
app.get('/api/seed-joue', async (req, res) => {
  try {
    const COUREURS_JOUE = [{"id": "joue_1964", "prenom": "Angélique", "nom": "Navet", "dossard": 1, "equipe": "ACTION ENFANCE", "event": "joue"}, {"id": "joue_2420", "prenom": "Fabien", "nom": "Teste", "dossard": 2, "equipe": "ACTION ENFANCE", "event": "joue"}, {"id": "joue_2382", "prenom": "Lucille", "nom": "DUPONT", "dossard": 3, "equipe": "ACTION ENFANCE", "event": "joue"}, {"id": "joue_2424", "prenom": "Marie", "nom": "MECHIN", "dossard": 4, "equipe": "ACTION ENFANCE", "event": "joue"}, {"id": "joue_2744", "prenom": "Océane", "nom": "SEEDORF", "dossard": 5, "equipe": "ACTION ENFANCE", "event": "joue"}, {"id": "joue_3003", "prenom": "Arouna", "nom": "Njoya Ngoueme", "dossard": 6, "equipe": "Agapè Anjou", "event": "joue"}, {"id": "joue_2041", "prenom": "Aurélien", "nom": "Le Foll", "dossard": 7, "equipe": "Agapè Anjou", "event": "joue"}, {"id": "joue_2995", "prenom": "Baya", "nom": "Friaa", "dossard": 8, "equipe": "Agapè Anjou", "event": "joue"}, {"id": "joue_2206", "prenom": "Bénédicte", "nom": "BUGUET", "dossard": 9, "equipe": "Agapè Anjou", "event": "joue"}, {"id": "joue_3001", "prenom": "Bogdan", "nom": "Vituk", "dossard": 10, "equipe": "Agapè Anjou", "event": "joue"}, {"id": "joue_3008", "prenom": "Delphine", "nom": "Dellobelle", "dossard": 11, "equipe": "Agapè Anjou", "event": "joue"}, {"id": "joue_3004", "prenom": "Ebenezer", "nom": "Arthur", "dossard": 12, "equipe": "Agapè Anjou", "event": "joue"}, {"id": "joue_2956", "prenom": "Elvina", "nom": "Jacquet", "dossard": 13, "equipe": "Agapè Anjou", "event": "joue"}, {"id": "joue_1665", "prenom": "Emmanuelle", "nom": "POITOU", "dossard": 14, "equipe": "Agapè Anjou", "event": "joue"}, {"id": "joue_2993", "prenom": "Ethan", "nom": "Deletang", "dossard": 15, "equipe": "Agapè Anjou", "event": "joue"}, {"id": "joue_2997", "prenom": "Hoda", "nom": "Bennouk Bouabidi", "dossard": 16, "equipe": "Agapè Anjou", "event": "joue"}, {"id": "joue_2998", "prenom": "Macka", "nom": "Abdelkhader Izzo", "dossard": 17, "equipe": "Agapè Anjou", "event": "joue"}, {"id": "joue_3007", "prenom": "Marie-Bénédicte", "nom": "Crépin", "dossard": 18, "equipe": "Agapè Anjou", "event": "joue"}, {"id": "joue_2994", "prenom": "Mehdi", "nom": "Seghaier", "dossard": 19, "equipe": "Agapè Anjou", "event": "joue"}, {"id": "joue_2992", "prenom": "Moussa", "nom": "Konaté", "dossard": 20, "equipe": "Agapè Anjou", "event": "joue"}, {"id": "joue_3005", "prenom": "Nathanael", "nom": "Godard", "dossard": 21, "equipe": "Agapè Anjou", "event": "joue"}, {"id": "joue_3002", "prenom": "Noa", "nom": "Motteau", "dossard": 22, "equipe": "Agapè Anjou", "event": "joue"}, {"id": "joue_2999", "prenom": "Nouah", "nom": "Takez", "dossard": 23, "equipe": "Agapè Anjou", "event": "joue"}, {"id": "joue_2052", "prenom": "patrick", "nom": "MUTZIG", "dossard": 24, "equipe": "Agapè Anjou", "event": "joue"}, {"id": "joue_3006", "prenom": "Rosalie", "nom": "Chatenoud", "dossard": 25, "equipe": "Agapè Anjou", "event": "joue"}, {"id": "joue_2996", "prenom": "Thomas", "nom": "De La Villeon", "dossard": 26, "equipe": "Agapè Anjou", "event": "joue"}, {"id": "joue_3000", "prenom": "Zacharia", "nom": "Mohamed Aden", "dossard": 27, "equipe": "Agapè Anjou", "event": "joue"}, {"id": "joue_2742", "prenom": "Luc", "nom": "PORTENSEIGNE", "dossard": 28, "equipe": "CGI France", "event": "joue"}, {"id": "joue_2622", "prenom": "Lucie", "nom": "Pessereau", "dossard": 29, "equipe": "CGI France", "event": "joue"}, {"id": "joue_2621", "prenom": "Nicolas", "nom": "MONANGE", "dossard": 30, "equipe": "CGI France", "event": "joue"}, {"id": "joue_2753", "prenom": "Quentin", "nom": "COMBEMOREL", "dossard": 31, "equipe": "CGI France", "event": "joue"}, {"id": "joue_2738", "prenom": "Sébastien", "nom": "ALLONCLE", "dossard": 32, "equipe": "CGI France", "event": "joue"}, {"id": "joue_2599", "prenom": "stephane", "nom": "sevingue", "dossard": 33, "equipe": "CGI France", "event": "joue"}, {"id": "joue_2105", "prenom": "Antoine", "nom": "Guerin", "dossard": 34, "equipe": "Compose - cantine sur mesure", "event": "joue"}, {"id": "joue_2187", "prenom": "Gabriel", "nom": "Bailly", "dossard": 35, "equipe": "Compose - cantine sur mesure", "event": "joue"}, {"id": "joue_1961", "prenom": "Yann", "nom": "TANGUY", "dossard": 36, "equipe": "Compose - cantine sur mesure", "event": "joue"}, {"id": "joue_2072", "prenom": "Séverine", "nom": "PAYOT", "dossard": 37, "equipe": "EGERIA - SAINT-CRICQ et Associés", "event": "joue"}, {"id": "joue_2071", "prenom": "Stanislas", "nom": "de LA RUFFIE", "dossard": 38, "equipe": "EGERIA - SAINT-CRICQ et Associés", "event": "joue"}, {"id": "joue_2073", "prenom": "Yves", "nom": "MOTTO", "dossard": 39, "equipe": "EGERIA - SAINT-CRICQ et Associés", "event": "joue"}, {"id": "joue_1953", "prenom": "JP", "nom": "Béchu", "dossard": 40, "equipe": "Esperancia", "event": "joue"}, {"id": "joue_2755", "prenom": "Blanche", "nom": "Dorrière", "dossard": 41, "equipe": "Fondation Rabelais", "event": "joue"}, {"id": "joue_2151", "prenom": "Marion", "nom": "Chemineau", "dossard": 42, "equipe": "Fondation Rabelais", "event": "joue"}, {"id": "joue_2664", "prenom": "Anthony", "nom": "LOUIS", "dossard": 43, "equipe": "Inserm 1253 iBraiN", "event": "joue"}, {"id": "joue_2377", "prenom": "Karen", "nom": "Ea", "dossard": 44, "equipe": "Inserm 1253 iBraiN", "event": "joue"}, {"id": "joue_2154", "prenom": "Antony", "nom": "Tellier", "dossard": 45, "equipe": "Institut départemental de l'Enfance et de la Famille (IDEF 37)", "event": "joue"}, {"id": "joue_2158", "prenom": "Didienne", "nom": "MACAIA", "dossard": 46, "equipe": "Institut départemental de l'Enfance et de la Famille (IDEF 37)", "event": "joue"}, {"id": "joue_2257", "prenom": "Emma", "nom": "TINNIERE", "dossard": 47, "equipe": "Institut départemental de l'Enfance et de la Famille (IDEF 37)", "event": "joue"}, {"id": "joue_2298", "prenom": "Florine", "nom": "Julien", "dossard": 48, "equipe": "Institut départemental de l'Enfance et de la Famille (IDEF 37)", "event": "joue"}, {"id": "joue_2147", "prenom": "Guillaume", "nom": "RUAUT", "dossard": 49, "equipe": "Institut départemental de l'Enfance et de la Famille (IDEF 37)", "event": "joue"}, {"id": "joue_2168", "prenom": "julie", "nom": "fournier", "dossard": 50, "equipe": "Institut départemental de l'Enfance et de la Famille (IDEF 37)", "event": "joue"}, {"id": "joue_2190", "prenom": "Katy-anna", "nom": "GRANGES", "dossard": 51, "equipe": "Institut départemental de l'Enfance et de la Famille (IDEF 37)", "event": "joue"}, {"id": "joue_2300", "prenom": "Mahamat", "nom": "Moussa Youssouf", "dossard": 52, "equipe": "Institut départemental de l'Enfance et de la Famille (IDEF 37)", "event": "joue"}, {"id": "joue_2299", "prenom": "Nadège", "nom": "Lorieux", "dossard": 53, "equipe": "Institut départemental de l'Enfance et de la Famille (IDEF 37)", "event": "joue"}, {"id": "joue_2260", "prenom": "nelly", "nom": "GALLERNE", "dossard": 54, "equipe": "Institut départemental de l'Enfance et de la Famille (IDEF 37)", "event": "joue"}, {"id": "joue_2149", "prenom": "Sabrina", "nom": "BAUGE", "dossard": 55, "equipe": "Institut départemental de l'Enfance et de la Famille (IDEF 37)", "event": "joue"}, {"id": "joue_2148", "prenom": "stephanie", "nom": "MORINIERE", "dossard": 56, "equipe": "Institut départemental de l'Enfance et de la Famille (IDEF 37)", "event": "joue"}, {"id": "joue_2215", "prenom": "Thomas", "nom": "Dussiaux", "dossard": 57, "equipe": "Institut départemental de l'Enfance et de la Famille (IDEF 37)", "event": "joue"}, {"id": "joue_2216", "prenom": "Vincent", "nom": "TREMBLIER", "dossard": 58, "equipe": "Institut départemental de l'Enfance et de la Famille (IDEF 37)", "event": "joue"}, {"id": "joue_2155", "prenom": "Ziriap", "nom": "Peter Ucin Cinda", "dossard": 59, "equipe": "Institut départemental de l'Enfance et de la Famille (IDEF 37)", "event": "joue"}, {"id": "joue_2818", "prenom": "Nicolas", "nom": "Marsande", "dossard": 60, "equipe": "Institut Départemental de l'Enfance et de la Famille d'Indre-et-Loire", "event": "joue"}, {"id": "joue_2048", "prenom": "Anais", "nom": "Oubella", "dossard": 61, "equipe": null, "event": "joue"}, {"id": "joue_2432", "prenom": "Fanny", "nom": "Klauk", "dossard": 62, "equipe": null, "event": "joue"}, {"id": "joue_1962", "prenom": "Hanane", "nom": "Aguidi", "dossard": 63, "equipe": null, "event": "joue"}, {"id": "joue_2082", "prenom": "Jérémy", "nom": "Brossier", "dossard": 64, "equipe": null, "event": "joue"}, {"id": "joue_3022", "prenom": "karine", "nom": "ASKAR", "dossard": 65, "equipe": null, "event": "joue"}, {"id": "joue_2773", "prenom": "lou", "nom": "savary", "dossard": 66, "equipe": null, "event": "joue"}, {"id": "joue_1957", "prenom": "Manon", "nom": "Salvadori", "dossard": 67, "equipe": null, "event": "joue"}, {"id": "joue_3011", "prenom": "Marie Luce", "nom": "Bierlaire", "dossard": 68, "equipe": null, "event": "joue"}, {"id": "joue_2042", "prenom": "Romain", "nom": "Alexandre", "dossard": 69, "equipe": null, "event": "joue"}, {"id": "joue_2304", "prenom": "Virginie", "nom": "Garnier", "dossard": 70, "equipe": null, "event": "joue"}, {"id": "joue_2212", "prenom": "Bertrand", "nom": "Verduzier", "dossard": 71, "equipe": "Les Cahutes de Louise", "event": "joue"}, {"id": "joue_2384", "prenom": "camille", "nom": "Verduzier", "dossard": 72, "equipe": "Les Cahutes de Louise", "event": "joue"}, {"id": "joue_3010", "prenom": "Nadia", "nom": "GUILLON", "dossard": 73, "equipe": "Les Cahutes de Louise", "event": "joue"}, {"id": "joue_3012", "prenom": "Patricia", "nom": "D'HERIN", "dossard": 74, "equipe": "Les Cahutes de Louise", "event": "joue"}, {"id": "joue_2374", "prenom": "Stephanie", "nom": "Jean-Baptiste", "dossard": 75, "equipe": "Les Cahutes de Louise", "event": "joue"}, {"id": "joue_3017", "prenom": "Adele", "nom": "HACOT", "dossard": 76, "equipe": "Réseau Entreprendre Loire Vallée", "event": "joue"}, {"id": "joue_3018", "prenom": "Agathe", "nom": "DE BRITO", "dossard": 77, "equipe": "Réseau Entreprendre Loire Vallée", "event": "joue"}, {"id": "joue_3019", "prenom": "Antonin", "nom": "LE ROY", "dossard": 78, "equipe": "Réseau Entreprendre Loire Vallée", "event": "joue"}, {"id": "joue_2383", "prenom": "Karine", "nom": "FARNAULT", "dossard": 79, "equipe": "Réseau Entreprendre Loire Vallée", "event": "joue"}, {"id": "joue_2268", "prenom": "Ahmed", "nom": "MARIN", "dossard": 80, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2278", "prenom": "Aïcha", "nom": "FARES", "dossard": 81, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2054", "prenom": "Alice", "nom": "Lecourt", "dossard": 82, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2624", "prenom": "Amaëlle", "nom": "Souchet", "dossard": 83, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2282", "prenom": "Amina", "nom": "DI MARIO", "dossard": 84, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2123", "prenom": "Annaïk", "nom": "LANNUZEL", "dossard": 85, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2178", "prenom": "Aurélie", "nom": "LE GALL", "dossard": 86, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2285", "prenom": "Camille", "nom": "BOSSELET", "dossard": 87, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2176", "prenom": "Catherine", "nom": "LAMBERT", "dossard": 88, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2063", "prenom": "Célia", "nom": "PEREIRA", "dossard": 89, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2280", "prenom": "Darlene", "nom": "REMILUS", "dossard": 90, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2173", "prenom": "Devillers", "nom": "DEVILLERS", "dossard": 91, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2182", "prenom": "Elodie", "nom": "ROCHE", "dossard": 92, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2267", "prenom": "Elys", "nom": "DELRUE", "dossard": 93, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2261", "prenom": "Erwan", "nom": "LANGER DELALOT", "dossard": 94, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2171", "prenom": "Estelle", "nom": "DAGONEAU", "dossard": 95, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2180", "prenom": "Florence", "nom": "NORTIER", "dossard": 96, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2043", "prenom": "Franck", "nom": "Esteban", "dossard": 97, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2191", "prenom": "Hélène", "nom": "MAILLARD", "dossard": 98, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2277", "prenom": "Inès", "nom": "RMICHE", "dossard": 99, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2274", "prenom": "Jaysy", "nom": "MARLOT GUINET", "dossard": 100, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2265", "prenom": "Jérémy", "nom": "BRAULT", "dossard": 101, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2276", "prenom": "Joudya", "nom": "EL RHAFFOULI", "dossard": 102, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2045", "prenom": "Julie", "nom": "POTICO", "dossard": 103, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2188", "prenom": "Karine", "nom": "BRIAND", "dossard": 104, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2263", "prenom": "Kengi", "nom": "MARY", "dossard": 105, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2174", "prenom": "Laëtitia", "nom": "FROGER", "dossard": 106, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2047", "prenom": "Linda", "nom": "Anthierens", "dossard": 107, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2184", "prenom": "Linda", "nom": "LELIEVRE-POIRIER", "dossard": 108, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2270", "prenom": "Loïc", "nom": "LANGER DELALOT", "dossard": 109, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2046", "prenom": "Lorène", "nom": "SAUVAGET", "dossard": 110, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2060", "prenom": "Lou", "nom": "Voltier", "dossard": 111, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2183", "prenom": "Lydie", "nom": "SEBERT", "dossard": 112, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2185", "prenom": "Margaux", "nom": "MICHEL", "dossard": 113, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2175", "prenom": "Marie", "nom": "JULLIEN", "dossard": 114, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2283", "prenom": "Monica", "nom": "FOSSI", "dossard": 115, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2272", "prenom": "Muhammad-Antaba", "nom": "MAZOUZI", "dossard": 116, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2279", "prenom": "Océane", "nom": "LANGER DELALOT", "dossard": 117, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2055", "prenom": "Ozanne", "nom": "Johana", "dossard": 118, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2273", "prenom": "Praise", "nom": "OMOBUDE", "dossard": 119, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2286", "prenom": "Salimatou", "nom": "TANDIAN", "dossard": 120, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2281", "prenom": "Salma", "nom": "DI MARIO", "dossard": 121, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2275", "prenom": "Sarrazin", "nom": "GOWNE", "dossard": 122, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2189", "prenom": "Sébastien", "nom": "BRIAND", "dossard": 123, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2181", "prenom": "Séverine", "nom": "REBINDAINE", "dossard": 124, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2264", "prenom": "Sofiane", "nom": "SEMACHE", "dossard": 125, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2284", "prenom": "Sonia", "nom": "CHAMPROUX", "dossard": 126, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2179", "prenom": "Sophie", "nom": "MARY", "dossard": 127, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2172", "prenom": "Stéphane", "nom": "DEVILLERS", "dossard": 128, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2177", "prenom": "Sylvie", "nom": "Fouret", "dossard": 129, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2170", "prenom": "Tailey", "nom": "VALLUET", "dossard": 130, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2059", "prenom": "TEA", "nom": "ANDREO", "dossard": 131, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2186", "prenom": "Tiphaine", "nom": "Locquet", "dossard": 132, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2269", "prenom": "Tom", "nom": "MARCEL", "dossard": 133, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2266", "prenom": "Tony", "nom": "MENNOCH", "dossard": 134, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2262", "prenom": "Yan", "nom": "DEVILLERS", "dossard": 135, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2271", "prenom": "Yazan", "nom": "EL RHAFFOULI", "dossard": 136, "equipe": "SAF Normandie - UPE", "event": "joue"}, {"id": "joue_2404", "prenom": "Anne-Lise", "nom": "MALINGE", "dossard": 137, "equipe": "Touraine le Département", "event": "joue"}, {"id": "joue_2388", "prenom": "Aurélie", "nom": "BATAILLE", "dossard": 138, "equipe": "Touraine le Département", "event": "joue"}, {"id": "joue_2393", "prenom": "Aurélie", "nom": "CARLOSEMA", "dossard": 139, "equipe": "Touraine le Département", "event": "joue"}, {"id": "joue_2406", "prenom": "Aurélie", "nom": "NIETO", "dossard": 140, "equipe": "Touraine le Département", "event": "joue"}, {"id": "joue_2386", "prenom": "Camille", "nom": "ANTIGNY", "dossard": 141, "equipe": "Touraine le Département", "event": "joue"}, {"id": "joue_2396", "prenom": "Cécile", "nom": "DARMENDRAIL", "dossard": 142, "equipe": "Touraine le Département", "event": "joue"}, {"id": "joue_2402", "prenom": "Emilie", "nom": "GOURRE", "dossard": 143, "equipe": "Touraine le Département", "event": "joue"}, {"id": "joue_2398", "prenom": "Florence", "nom": "FARAJ", "dossard": 144, "equipe": "Touraine le Département", "event": "joue"}, {"id": "joue_2415", "prenom": "Franck", "nom": "DHONT", "dossard": 145, "equipe": "Touraine le Département", "event": "joue"}, {"id": "joue_2414", "prenom": "François", "nom": "CHARTIER", "dossard": 146, "equipe": "Touraine le Département", "event": "joue"}, {"id": "joue_2395", "prenom": "Françoise", "nom": "CHENE", "dossard": 147, "equipe": "Touraine le Département", "event": "joue"}, {"id": "joue_2390", "prenom": "Gaëlle", "nom": "BODIOU", "dossard": 148, "equipe": "Touraine le Département", "event": "joue"}, {"id": "joue_2399", "prenom": "Gwenola", "nom": "FETU", "dossard": 149, "equipe": "Touraine le Département", "event": "joue"}, {"id": "joue_2385", "prenom": "Hanane", "nom": "AKID", "dossard": 150, "equipe": "Touraine le Département", "event": "joue"}, {"id": "joue_2413", "prenom": "Joël", "nom": "BATARD", "dossard": 151, "equipe": "Touraine le Département", "event": "joue"}, {"id": "joue_2394", "prenom": "Julie", "nom": "CHAMAILLARD", "dossard": 152, "equipe": "Touraine le Département", "event": "joue"}, {"id": "joue_2416", "prenom": "Julien", "nom": "DUPORTAL", "dossard": 153, "equipe": "Touraine le Département", "event": "joue"}, {"id": "joue_2389", "prenom": "Laure", "nom": "BERNEZ", "dossard": 154, "equipe": "Touraine le Département", "event": "joue"}, {"id": "joue_2391", "prenom": "Laure", "nom": "BONRAISIN", "dossard": 155, "equipe": "Touraine le Département", "event": "joue"}, {"id": "joue_2405", "prenom": "Louise", "nom": "NAUDON", "dossard": 156, "equipe": "Touraine le Département", "event": "joue"}, {"id": "joue_2400", "prenom": "Maëlle", "nom": "FORT", "dossard": 157, "equipe": "Touraine le Département", "event": "joue"}, {"id": "joue_2410", "prenom": "Marianne", "nom": "RENAUD", "dossard": 158, "equipe": "Touraine le Département", "event": "joue"}, {"id": "joue_2411", "prenom": "Marie", "nom": "ROUSSE", "dossard": 159, "equipe": "Touraine le Département", "event": "joue"}, {"id": "joue_2408", "prenom": "Marjolaine", "nom": "PASQUIER", "dossard": 160, "equipe": "Touraine le Département", "event": "joue"}, {"id": "joue_2403", "prenom": "Mélissa", "nom": "LOUSSOUARN", "dossard": 161, "equipe": "Touraine le Département", "event": "joue"}, {"id": "joue_2392", "prenom": "Morgane", "nom": "BRECHELIERE", "dossard": 162, "equipe": "Touraine le Département", "event": "joue"}, {"id": "joue_2401", "prenom": "Myriam", "nom": "GANDAIS", "dossard": 163, "equipe": "Touraine le Département", "event": "joue"}, {"id": "joue_2387", "prenom": "Nadia", "nom": "ARCHAMBAULT", "dossard": 164, "equipe": "Touraine le Département", "event": "joue"}, {"id": "joue_2290", "prenom": "Nathalie", "nom": "Gouin", "dossard": 165, "equipe": "Touraine le Département", "event": "joue"}, {"id": "joue_2397", "prenom": "Sabine", "nom": "ESSEUL", "dossard": 166, "equipe": "Touraine le Département", "event": "joue"}, {"id": "joue_2409", "prenom": "Sandrine", "nom": "PEREIRINHA", "dossard": 167, "equipe": "Touraine le Département", "event": "joue"}, {"id": "joue_2417", "prenom": "Sébastien", "nom": "FOURASTE", "dossard": 168, "equipe": "Touraine le Département", "event": "joue"}, {"id": "joue_2418", "prenom": "Thomas", "nom": "GEDEON", "dossard": 169, "equipe": "Touraine le Département", "event": "joue"}, {"id": "joue_2407", "prenom": "Valérie", "nom": "PASCOA", "dossard": 170, "equipe": "Touraine le Département", "event": "joue"}, {"id": "joue_2412", "prenom": "Virginie", "nom": "WINIARZ", "dossard": 171, "equipe": "Touraine le Département", "event": "joue"}, {"id": "joue_2564", "prenom": "Christelle", "nom": "LE FLOCH", "dossard": 172, "equipe": "WORKNETT", "event": "joue"}, {"id": "joue_2563", "prenom": "Daniel", "nom": "RICHARD", "dossard": 173, "equipe": "WORKNETT", "event": "joue"}, {"id": "joue_2562", "prenom": "Saidou", "nom": "NACANABO", "dossard": 174, "equipe": "WORKNETT", "event": "joue"}];
    ...
    res.json({ success: true, total: COUREURS_JOUE.length, redis: ok });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});

// GET /api/search-joue — recherche dossards Joué depuis Redis
app.get('/api/search-joue', async (req, res) => {
  try {
    const { q, type } = req.query;
    if (!q || q.trim().length < 2)
      return res.status(400).json({ error: 'Au moins 2 caractères requis.' });

    const raw = await redisGet('defi_enfance_dossards_joue');
    if (!raw) return res.json({ results: [], count: 0, query: q });
    ...
    if (type === 'equipe') {
      ...
      results = coureurs
        .filter(c => c.nom.toLowerCase().includes(term) || c.prenom.toLowerCase().includes(term))
        .sort((a, b) => a.nom.localeCompare(b.nom, 'fr'));
    }

    res.json({ results, count: results.length, query: q.trim() });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});

// GET /api/equipes-joue — liste des équipes Joué
app.get('/api/equipes-joue', async (req, res) => {
  try {
    const raw = await redisGet('defi_enfance_dossards_joue');
    if (!raw) return res.json({ equipes: [] });
    const coureurs = JSON.parse(raw);
    const equipes = [...new Set(coureurs.map(c => c.equipe).filter(Boolean))].sort((a, b) => a.localeCompare(b, 'fr'));
    res.json({ equipes });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.listen(PORT, async () => {
  ...
  // Auto-refresh résultats si Redis vide
  try {
    const raw = await redisGet('defi_enfance_resultats');
    if (!raw) {
      console.log('Redis résultats vide — rechargement automatique...');
      const data = await loadResultats();
      await saveToRedis_key('defi_enfance_resultats', JSON.stringify(data), 7 * 24 * 60 * 60);
    } else {
      console.log('Résultats déjà en cache Redis.');
    }
  } catch(e) {
    console.log('Auto-refresh résultats erreur:', e.message);
  }
});
